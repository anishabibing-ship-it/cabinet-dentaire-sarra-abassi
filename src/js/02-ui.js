...
const App = {
  ...
  showAuth() {
    ...
    hint.innerHTML = (seeded
      ...
      : `En cas d'oubli du mot de passe, restaurez une sauvegarde depuis un autre poste.`)
      + `<br><br>Stockage : ${Store.mode === 'memory'
          ? `<b style="color:var(--bad)">indisponible — rien ne sera conservé</b>`
          : U.esc(Store.label())}.`;
  ...
  showApp() {
    ...
    /* Le navigateur refuse tout stockage : l'avertir sans ambiguïté et en permanence */
    if (Store.mode === 'memory' && !U.$('#memWarn')) {
      const b = document.createElement('div');
      b.id = 'memWarn';
      b.className = 'alert-band no-print';
      b.style.cssText = 'border-radius:0;border-width:0 0 1px 0';
      b.innerHTML = `${Icons.alert}<span><b>Le navigateur refuse le stockage local.</b>
        Tout ce que vous saisirez sera perdu à la fermeture de l'onglet. Ouvrez l'application dans une
        fenêtre ordinaire (hors navigation privée), ou installez le fichier <code>cabinet-erp.html</code>
        sur le poste du cabinet. Exportez une sauvegarde avant de fermer.</span>`;
      U.$('.main').insertBefore(b, U.$('#page'));
    }
